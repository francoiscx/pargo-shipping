...
A1: This plugin enables visiters to your store to select PARGO as a shipping method.
This plugin will then apply a flat rate of R59.95 to the order as shipping cost and requet a PARGO Point where the order need to be sent to. This also assumes a delivery time of 7-9 working days which is indicated on the checkout page.
...
Q5: How will the clinets know what the code of their nearest PARGO Point is?
...
2. Checkout with PARGO | When PARGO is selected at checkout,a  Input field for PARGO Point will be displayed (1), To locate the nearest point the user can click on "Locate nearest PARGO Point". In the event where the PARGO Point was not spesified and error will be displayed for the user, with instructions (3).
